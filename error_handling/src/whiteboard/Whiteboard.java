...
public class Whiteboard extends JFrame implements ActionListener {
	...
	private void closingAction() {
		...
		if (ans == JOptionPane.YES_OPTION) {
			...
			//when close the white board, we need to release socket
			JSONObject json = new JSONObject();
		    json.put("header", "close");
			json.put("body", chatbox.getName());   //close the client according to client name
			
			OutputStreamWriter writer;
			try {
				
				writer = new OutputStreamWriter(chatbox.getSocket().getOutputStream(), "UTF-8");
				writer.write(json.toString() + "\n");
				writer.flush();              //send the json to server
